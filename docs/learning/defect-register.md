...
**Investigation and cause:** A fixed frame count does not imply a fixed elapsed duration. Exploration records 97 stride-four frames averaging 16.675258 ms, with scaled movement speed still approximately .22. Mechanism-input records baseline150 mean 4.166667 ms, active600 mean 4.8125 ms and 598 moving frames. Their assertions detect insufficient stimulus, not a runtime clamp or performance-budget defect. Exact C source/build and both failed evidence archives remain retained under `output/manager/phase10-integration/`.

**Correction and check:** Source-admitted R2 adds adaptive observation/command windows and a development-only synchronous CPU update/render-submission recorder. Its 150/600-frame mean limits are 8/9 ms, p95 12 ms, maximum 40 ms and at most 5% strictly above 20 ms. Paired CPU mean difference and native intervals are diagnostics only; no GPU-completion claim is made. The user's low/high/changing-cadence requirement supersedes v2's thresholds, preserved as history. The mandatory CPU proofs exercise the actual runtime clamp, observation helper, recorder and command coverage, including mutations and changing cadence. They collect sufficient observations before asserting behavior. Native low-cadence ambiguity is reported explicitly, with the CPU mutation proof required. The original repair package also exposed shared emitter state: its preflight consumed sequence values 1–4 before the real run. R2 isolates that proof module; the exact old first-row 5 fails and the new first-row 1 passes the unchanged strict parser.

**Integrated verification:** All six affected R2 commands pass with unchanged source/build. Exploration completes 13 groups; mechanism-input completes 20 groups and 33 captures, with fourteen trusted commands distributed across all six active-window portions and 592 moving frames. CPU baseline/active means are 1.715333/2.208500 ms, p95 2/2.5 ms and maxima 2.2/2.9 ms, with no work sample above 20 ms. One hundred real rail cycles preserve resources; errors and budget violations are empty. Evidence `956251f9c007035cda012a6954c00c232c15101e2bcee3c9699af639240436a0` remains under ignored `output/manager/phase10-integration/mechanism-input-r2/`. The source finding is closed; final integrated review 41 and manager phase acceptance remain pending. The durable requirement is in `docs/policies/local-rules.md`.

## Gardening guidance must precede the response (F32)

**Reported symptom:** Independent native review 35 found that A2/B gardeners watered and acknowledged before the helper pointed. Valid contact geometry did not make the helper's contribution read as cooperation.

**Investigation and cause:** The authoritative gardening stage array placed `help` last. The existing poses already supplied soil-directed pointing, attention changes and can movement, but their order put guidance after tending. Original A2/B source, native recordings and the review remain retained.

**Correction and check:** Candidate C moves only `help` before `water-prepare`, preserving the 9.1-second activity duration, geometry, props, approaches and supported circuit positions. `check:social-state` and `check:social-contact` require observed guidance, preparation, water, drain, lowering and acknowledgment in that order for both real pairs. Their 240-second runs check all 16 completed garden cycles; detailed emitted gardening geometry covers selected samples from the first completed cycle of each pair, not 16 full mesh sweeps. Both gates execute the exact old B model, require actual completed cycles and reject its order despite valid contact geometry. The manager closes F32 through source review 37 and independent native review 38's 20 original-resolution PNGs and 12 decoded frames. Six separately inspected manager help/water/acknowledgment views support that bounded judgment. Gestures remain quiet; neither video was watched continuously, and the CPU checks alone do not establish readability. Final root input/lifecycle/performance acceptance remains pending.

## Equal paused times must not replay social history (F31)

**Reported symptom:** Independent source review 33 found that three paused updates at raw time `10.1234567896` reset the social model three times and replayed 909 fixed ticks while snapshots and poses remained identical. A rounded-up explicit seek could also replay twice. Work grew with elapsed simulated time without a visible state change.

**Investigation and cause:** The community wrapper compared raw caller time with the model's rounded nanosecond time and treated an unchanged rounded-up value as a backward seek. State-equality and pixel-equality checks could not detect the repeated work. Original A2 and its count-based failure remain retained.

**Correction and check:** B compares time at the model's nanosecond precision while preserving raw-input validation, then presents an already-seeked or restored frame without routing it through another update. `check:social-time` counts actual model resets and steps across the original two fractions, equal canonical nanoseconds, true backward update, explicit seek, restore, partitions and invalid raw input. Reinstating the raw comparison must produce and reject the original three resets/909 ticks. The manager accepts focused review 36; unchanged B repair inputs and a separately source-bound passing C time report carry the repair into C. Zero replay means zero model resets/ticks at equal time; presentation still runs. Final root browser and timing acceptance remain pending.
